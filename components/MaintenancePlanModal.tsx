...
const MaintenancePlanModal: React.FC<MaintenancePlanModalProps> = ({ plan, onSave, onClose, allRepairs }) => {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        if (!formData.vehicleLicensePlate || !formData.planName) {
            addToast('กรุณากรอกทะเบียนรถและชื่อแผน', 'warning');
            return;
        }
        onSave(formData);
    };
                ...
                <form id="maintenance-plan-form" onSubmit={handleSubmit} className="p-6 space-y-4 overflow-y-auto">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <input type="text" name="planName" value={formData.planName} onChange={handleInputChange} required aria-label="ชื่อแผน" className="w-full p-2 border border-gray-300 rounded-lg" placeholder="เช่น เปลี่ยนน้ำมันเครื่อง" />
                        </div>
                    </div>
                    ...
                    <div className="p-4 border rounded-lg bg-gray-50">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                ...
                                <input type="date" name="lastServiceDate" value={formData.lastServiceDate} onChange={handleInputChange} aria-label="วันที่เข้ารับบริการล่าสุด" className="w-full p-2 border border-gray-300 rounded-lg" />
                            </div>
                            <div>
                                <label className="block text-sm font-medium text-gray-700 mb-1">ความถี่</label>
                                <div className="flex gap-2">
                                    <input type="number" name="frequencyValue" value={formData.frequencyValue} onChange={handleInputChange} aria-label="ความถี่" className="w-1/2 p-2 border border-gray-300 rounded-lg" />
                                    <select name="frequencyUnit" value={formData.frequencyUnit} onChange={handleInputChange} aria-label="หน่วยความถี่" className="w-1/2 p-2 border border-gray-300 rounded-lg">
                    ...
                    <div className="p-4 border rounded-lg bg-gray-50">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                ...
                                <input type="number" name="lastServiceMileage" value={formData.lastServiceMileage} onChange={handleInputChange} aria-label="เลขไมล์ล่าสุด" className="w-full p-2 border border-gray-300 rounded-lg" />
                            </div>
                            <div>
                                <label className="block text-sm font-medium text-gray-700 mb-1">ระยะทางสำหรับครั้งถัดไป</label>
                                <input type="number" name="mileageFrequency" value={formData.mileageFrequency} onChange={handleInputChange} aria-label="ระยะทางสำหรับครั้งถัดไป" className="w-full p-2 border border-gray-300 rounded-lg" placeholder="เช่น 10000" />
                            </div>
                        </div>
                    </div>
